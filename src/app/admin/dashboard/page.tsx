...
export default async function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
      ...
      {/* Admin Header */}
      <header className="bg-slate-900 text-white p-6 sticky top-0 z-50 shadow-md flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
            <Wrench className="w-5 h-5 text-blue-400" /> Admin Console
          </h1>
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center">
            ...
            <button className="text-sm text-blue-600 hover:underline font-medium">Refresh Data</button>
          </div>
                ...
                {inquiries.map((job) => (
                  <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <Link href={`/admin/jobs/${job.id}`} className="text-blue-600 hover:text-blue-800 font-bold text-xs bg-blue-50 px-3 py-1 rounded-full">
                        View
                      </Link>
                    </td>
                  </tr>
